  ...
  <div :class="['brand-table',className]">
    <template v-if="!noData">
        ...
        <div v-for="(item, index) in tableColumns"
          ...
          <template v-if="item.sortable">
            <div class="sort-icon-wrap">
              <div name="play"
                   class="triangle up"
                   :class="{'sort-active': item.sortOrder==='asc'}">
              </div>
              <div name="play"
                   class="triangle down"
      ...
      <div class="table-body">
        <div v-for="(item, rowIndex) in tableData"
             :key="rowIndex"
             class="body-row"
             @click="rowClick(item)">
          <div v-for="(column, index) in tableColumns"
               ...
               :style="{'width': column.width, 'text-align': column.textAlign}">
            <template v-if="column.dataIndex==='index'">
              <slot :name="column.customerSlot"
                    :scope="{data:item[column.dataIndex],rowIndex:rowIndex, rowData: item}">
      ...
      <div v-if="maxLength<data.length"
           class="table-footer"
           @click="showMore">
        {{ moreData ? '收起' : '展开更多' }}
        <van-icon v-if="moreData"
                  name="arrow-up"/>
        <van-icon v-else
                  name="arrow-down"/>
      </div>
